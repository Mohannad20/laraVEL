...
<div class="container mx-auto p-4">
    ...
    <form action="{{ route('villes.update', $ville->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="ville" id="ville" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('ville', $ville->ville) }}" required>
        </div>
        <div class="mb-4">
            <label for="nombreHabitant" class="block text-gray-700 text-sm font-bold mb-2">Nombre d'Habitants</label>
            <input type="number" name="nombreHabitant" id="nombreHabitant" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('nombreHabitant', $ville->nbrHabitants) }}" required>
        </div>
